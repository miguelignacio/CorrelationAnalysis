...
int main(int argc, char *argv[])
{
  ...
  //FIXME: Add to config file
	...
	if (isolation<iso_max){	
	  if (Signal){
	    ...
	    for (double Iso_bin = -0.5; Iso_bin < iso_max; Iso_bin += 0.5)
	      if (isolation > Iso_bin && isolation < Iso_bin+0.5) Signal_pT_Dist->Fill(cluster_pt[n],isolation,weight);
	    for (int ipt = 0; ipt < nptbins; ipt++)
	      if (cluster_pt[n] >ptbins[ipt] && cluster_pt[n] <ptbins[ipt+1])
		H_Signal_Triggers[ipt]->Fill(1.0,weight);
	  }
	  //Low DNN Trigger BKGD
	  if (Background){
	    h_ntrig.Fill(1.0,weight);
	    N_BKGD_Triggers += 1;
	    for (double Iso_bin = -0.5; Iso_bin < iso_max; Iso_bin += 0.5)
	      if (isolation > Iso_bin && isolation < Iso_bin+0.5) 
		BKGD_pT_Dist->Fill(cluster_pt[n],isolation,weight);
	    for (int ipt = 0; ipt < nptbins; ipt++)
	      if (cluster_pt[n] >ptbins[ipt] && cluster_pt[n] <ptbins[ipt+1]) 
		H_BKGD_Triggers[ipt]->Fill(1.0,weight); 
	  }
	}
	//no iso, no dnn
	for (int ipt = 0; ipt < nptbins; ipt++)
	  Triggers[ipt]->Fill(0.0,weight);
	...
	//fprintf(stderr,"\n"); 
      ...
      // if (ievent%1000 == 0)
      // 	std::cout<<std::endl<<weight_total<<std::endl;
  ...
  BKGD_pT_Dist->Write();
  Lambda_0_Distro.Scale(1/weight_total);
  Lambda_0_Distro.Write();
  for (int ipt = 0; ipt<nptbins; ipt++){
  ...
  for (int ipt = 0; ipt<nptbins; ipt++){
    ...
    for (int ipt = 0; ipt<nptbins; ipt++){
      for (int izt = 0; izt<nztbins; izt++){
	//Corr[izt+ipt*nztbins]->Scale(H_Signal_Triggers[ipt]->Integral(0,-1));
	Corr[izt+ipt*nztbins]->Write();}
      for (int izt = 0; izt<nztbins; izt++){
	//IsoCorr[izt+ipt*nztbins]->Scale(H_Signal_Triggers[ipt]->Integral(0,-1));  
	IsoCorr[izt+ipt*nztbins]->Write();}
      for (int izt = 0; izt<nztbins; izt++){
	//BKGD_IsoCorr[izt+ipt*nztbins]->Scale(H_Signal_Triggers[ipt]->Integral(0,-1));  
