...
void Run(const int TrackBit, char* filename){
  ...
  for (Long64_t ievent=0;ievent< tree->GetEntries();ievent++) {
    ...
    //Loop to ver fraction of reconstructed ITS-only tracks that are not associated to any MC primary
      for (int n=0; n< ntrack; n++){ //loop over reconstructed tracks
	  if( (track_quality[n]&TrackBit)!=0) {
	    hReco->Fill(track_pt[n]);
	    if(track_mc_truth_index[n]==65535) hFake->Fill(track_pt[n]);
	   }
	} //end loop of tracks
  ...
  std::cout << " END LOOP  " << std::endl;
   ...
   for(int i=0; i<nbins; i++){
     ...
     c1->SaveAs(Form("projecting%i_TrackBit%i_%s.pdf", i, TrackBit,filename));
   }
   ...
   //   
   auto eff = new TGraphAsymmErrors(hNum, hDen);
   eff->SetTitle("; p_{T} ; #epsilon"); 
   ...
   gPad->SetLogz();
   hCorrelation_dEtapT->GetYaxis()->SetNdivisions(5);
   hCorrelation_dEtapT->GetXaxis()->SetNdivisions(5);
   c->SaveAs(Form("Correlation_deta_pt%i_%s.pdf",TrackBit,filename));

   c->Clear();
   hCorrelation_dPhipT->Draw("colz");
   gPad->SetLogz();
   hCorrelation_dPhipT->GetYaxis()->SetNdivisions(5);
   hCorrelation_dPhipT->GetXaxis()->SetNdivisions(5);
   c->SaveAs(Form("Correlation_dphi_pt%i_%s.pdf",TrackBit,filename));
   ...
   c->SaveAs(Form("MomentumCorrelation_dphi_pt%i_%s.pdf",TrackBit,filename));


   c->Clear();
   hNum2Deta->Draw("colz");
   hNum2Deta->SetTitle("; #eta ; p_{T} [GeV]");
   gPad->SetLogz(0);
   gPad->SetLogy(1);
   c->SaveAs(Form("Eff_Eta_%i_%s.pdf",TrackBit,filename));
   c->Clear();
   hNum2Dphi->Draw("colz");
   hNum2Dphi->SetTitle("; #phi ; p_{T} [GeV]");
   gPad->SetLogz(0);
   gPad->SetLogy(1);
   c->SaveAs(Form("Eff_phi_%i_%s.pdf",TrackBit,filename));
...
void RunOverMC(){
  Run(16,"woSDDTuple.root");//Select Trackbit 16, which is ITS-only.
  Run(3, "woSDDTuple.root"); //Select track bit 3, which is ITS-TPC AND PWG-JE cuts
